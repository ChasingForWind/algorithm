public class Demo35_2_test {
    public static void main(String[] args) {
        int[] array = {5};
        int i = GetNumberOfK(array, 0);
        ...
        public static int GetNumberOfK(int [] array , int k) {
            ...
            int leftK = getLeftIndex(array,k,0,array.length);
            //递归找到相同的数字的最右边一个ringtK
            int rightK = getRightIndex(array,k,0,array.length);
            //返回具体数值
            if (leftK!=-1&&rightK!=-1){
        ...
        public static int getLeftIndex(int [] array,int k,int start,int end){
            //出口1
        ...
        public static int getRightIndex(int [] array,int k,int start,int end){
            //出口1
            if(start>end) return -1;
            //定义mid值
            int mid = (start+end)>>1;
